...
label "tag signing"
# tag.gpgsign is true globally, so a bare `git tag <name>` is signed and therefore annotated.
# --no-sign is the documented per-invocation override that puts a lightweight tag back on the
# table, so it is the tag-side twin of `git commit --no-gpg-sign`.
assert deny "git tag --no-sign v1.0.0"
assert deny "git tag v1.0.0 --no-sign"
assert deny "cd repo && git tag --no-sign v1.0.0"
assert deny "git -C repo tag --no-sign v1.0.0"
assert allow "git tag -a v1.0.0 -m 'release'"
assert allow "git tag -s v1.0.0 -m 'release'"
assert allow "git tag"
assert allow "git tag -l 'v1.*'"
# Deliberately not guarded - see the guard's header. These pin the choice, not an oversight.
assert allow "git tag -d v1.0.0"
assert allow "git tag -f v1.0.0"
...
assert deny "git -c tag.gpgsign=false tag v1.0.0"
assert deny "git -c tag.gpgSign=0 tag v1.0.0"
assert deny "git -c tag.gpgsign=off tag v1.0.0"
assert deny "git -ctag.gpgsign=no tag v1.0.0"
assert deny "git --config-env=tag.gpgsign=NOPE tag v1.0.0"
assert deny "GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=tag.gpgsign GIT_CONFIG_VALUE_0=false git tag v1.0.0"
assert allow "git -c commit.gpgsign=true commit -m hello"
assert allow "git -c tag.gpgsign=true tag v1.0.0"
...
assert deny "git config tag.gpgsign false"
assert deny "git config --global tag.gpgsign false"
assert deny "git config --local tag.gpgSign 0"
assert deny "git config --global tag.gpgsign off"
assert deny "git config set tag.gpgsign no"
assert deny "git config --global --unset tag.gpgsign"
assert deny "git config --unset-all tag.gpgsign"
assert deny "git config unset tag.gpgsign"
...
assert allow "git config --global tag.gpgsign true"
assert allow "git config --get tag.gpgsign"
assert allow "git config --get tag.gpgsign --global"
assert allow "git config get commit.gpgsign"
